...
\State \textbf{StateflowContent} := Name, Path, Data, States, Junctions, TruthTables
...
\State \textbf{Inport} := \{\textit{Positive integer}\}+  \ \ \ \ \ \ \ \ \ \ \ \ \  \textbf{Outport} := \{\textit{Positive integer}\}+ 
\State \textbf{\textsf{Inport}} := \textit{Data type}*  \ \ \ \ \ \ \ \ \ \ \ \  \textbf{\textsf{Outport}} := \textit{Data type}*
...
\State \textbf{DstBlock} := \textit{Handle}*  \ \ \ \ \ \ \ \ \ \ \ \  \textbf{SrcBlock} := \textit{Handle}*
...
Stateflow can also be used in CoCoSim. 
They are translated as the value for \textsf{StateflowContent} in JSON.
Figure~\ref{sfjsongrammar} shows only relevant grammar used for defining Stateflow in JSON.


\begin{figure}
\begin{algorithmic}
\State \textbf{StateflowContent} := Name, Path, Data, States, Junctions, TruthTables
\State \textbf{Id} := \textit{String}
\State \textbf{Path} := \textit{String}
\State \textbf{Name} := \textit{String}
\State \textbf{Data} := \{Id, Name, Datatype, CompiledType, Port, InitialValue, Scope, ArraySize\}*
\State \textbf{States} := \{Path, Id, Name, Actions, InnerTransitions, OuterTransitions, Composition\}*
\State \textbf{Junctions} := \{Path, Name, Id, Type, OuterTransitions\}*
\State \textbf{DefaultTransitions} := \{Destination\}*
\State \textbf{OuterTransitions} := \{Destination\}*
\State \textbf{Substates} := \{\textit{String}\}*
\State \textbf{States} := \{Unsigned integer\}*
\State \textbf{Destination} := \{Id, DestinationStateJunction, Condition, ConditionAction, TransitionAction\}*
\State \textbf{Composition} := \{Type, DefaultTransitions, Substates, States\}*
\State \textbf{Condition} := \{Matlab Expressions in String format\}*
\State \textbf{ConditionAction} := \{Matlab Expressions in String format\}*
\State \textbf{TransitionAction} := \{Matlab Expressions in String format\}*
\State \textbf{DestinationStateJunction} := \{Id, Type, Name\}*
\State \textbf{Type} := \{EXCLUSIVE\_OR $\mid$ PARALLEL\_AND $\mid$ Junction $\mid$ State\}


\end{algorithmic}
\caption{The grammar for the JSON representation of Stateflow}
\label{sfjsongrammar}
\end{figure}
...
\end{verbatim}
\caption{The Schema for a JSON representation of a CoCoSim model}
\end{figure}
