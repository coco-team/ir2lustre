...
\State \textbf{Constant} := \textit{Decimal number $\mid$ Unsigned integer $\mid$ Boolean constant}
\State \textbf{Inputs} := \{$+^*-^*+^*-^*$\} $\mid$ \{$\times^* /^*\times^* /^*$\} $\mid$ \textit{Unsigned integer}
...
\State \textbf{\textsf{Inport}} := Data type*
\State \textbf{\textsf{Outport}} := Data type*
...
\State \textbf{DstBlock} := Handle*
\State \textbf{SrcBlock} := Handle*

\end{algorithmic}
\caption{The grammar for the JSON representation of CoCoSim model}
\label{jsongrammar}
\end{figure}
