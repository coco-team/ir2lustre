...
we can use them create a node to represent the operator \textsf{floor} as $floor = \textsf{real} (\textsf{int}$ $x)$, where $x$ and $floor$ are both real numbers and $floor$ is the result of applying \textsf{floor} on $x$. 
And the \textsf{ceil} block can be represented calling node \textsf{floor} as $ceil = -\textsf{floor}(-x)$.
Round block is translated to a node \textsf{round} with its body defined as $round = \textsf{if}$ $x >= 0.0$ $\textsf{then}$ $\textsf{ceil}(\textsf{abs}(x))$ \textsf{else} $-\textsf{ceil}(\textsf{abs}(x))$, where \textsf{abs} is a Lustre library node to compute the absolute value of the input.
Fix block is translated to a node \textsf{fix} with its body defined as $fix = \textsf{if}$ $x >= 0.0$ $\textsf{then}$ $\textsf{floor}(\textsf{abs}(x))$ \textsf{else} $-\textsf{floor}(\textsf{abs}(x))$, where \textsf{abs} is a Lustre library node to compute the absolute value of the input.
...
For data types conversions from bool to integer(real), we use library nodes \textsf{bool\_to\_int, bool\_to\_real, int\_to\_bool} and \textsf{real\_to\_bool} respectively, where \textsf{bool\_to\_int} and \textsf{bool\_to\_real} are defined as $output =$ \textsf{if} $input$ \textsf{then} $1(1.0)$ \textsf{else} $0(0.0)$
...
\subsection{Unsupported Blocks}

Currently, we do not support translation for vector-related computations, such as \textsf{Vector Concatenate, Selector, Demux, Mux, vector plus/minus/multiplications} and so on. 
We also do not support nonlinear blocks, such as \textsf{sqrt, log, exp}, and \textsf{Rounding} blocks. 
We partially support \textsf{data type conversion} block for converting from Boolean to Integer and Real, and Integer and Real to Boolean. 
However, we would ignore conversions related to the different bits integer and different precision real numbers and so on.
