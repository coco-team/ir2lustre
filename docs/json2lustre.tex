...
Stateflow is translated into \href{http://kind.cs.uiowa.edu/kind2_user_doc/#n3801905-hierarchical-automata} {Lustre automaton} based on its semantics. 

\subsubsection{Translation of States}

Each state has nine different actions i.e., Entry, During, Exit, Bind, On, OnAfter, OnBefore, OnAt, OnEvery. 
Among those, the translation currently only supports Entry, During and Exit actions and does not support event. 
State is translated into an automaton state in Lustre with its during actions as its body equations.

\subsubsection{Translation of Junctions}

Junctions are compressed as simple transitions to states. 
A simple transition is defined as a transition from a state to another based on some condition.
